import sys, os, threading, time, uvicorn, numpy as np
...
from src.metrics.anomaly import detect_adversarial_flood, detect_adaptive_replay_anomaly
...
def main():
    t = threading.Thread(target=run_api, daemon=True); t.start(); time.sleep(2)
    ...
    broker = DecisionBroker(); gov = TriGovernor()
