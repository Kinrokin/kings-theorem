...
from src.algebra.constraint_lattice import ConstraintLattice, Constraint
from src.ethics.manifold import ManifoldProjector, EthicalManifold
...
class KernelOrchestrator:
    ...
    def run_dialectic(self, user_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for kernel in self.kernels:
            ...
            steps.append(StepResult(step_id=kernel.name, verdict=r.get("verdict","CONTINUE"), output_artifact=r.get("artifact",{}), claims=r.get("claims",[])))
            # update current for next step
            current = r.get("artifact", current)
        bundle = PCEBundle(bundle_id="pce-"+user_input.get("id","0"), initial_input_hash="init", steps=steps, final_output_hash="final")
        # Local veto check
