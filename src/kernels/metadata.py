...
def sign_with_ed25519(privkey_pem: bytes, payload: bytes) -> str:
    priv = Ed25519PrivateKey.from_private_bytes(privkey_pem) if isinstance(privkey_pem, bytes) and len(privkey_pem)==32 else Ed25519PrivateKey.from_private_bytes(privkey_pem)
...
# Public API
def sign_metadata(metadata: KernelMetadata, privkey: Optional[bytes] = None, hmac_secret: Optional[bytes] = None) -> KernelMetadata:
...
def verify_metadata(metadata: KernelMetadata, pubkey: Optional[bytes] = None, hmac_secret: Optional[bytes] = None) -> bool:
