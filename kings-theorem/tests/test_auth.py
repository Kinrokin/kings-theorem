...
def test_token_enforced(monkeypatch):
    ...
    # Wrong token
    r = client.post("/v1/generate", json={"prompt": "hello"}, headers={"Authorization": "Bearer wrong"})
    assert r.status_code == 401

    # Correct token via Bearer
    r = client.post("/v1/generate", json={"prompt": "hello"}, headers={"Authorization": "Bearer secret-token"})
    assert r.status_code == 200
