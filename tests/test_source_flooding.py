...
def test_source_registration_flood_detection():
    ...
    for i in range(3):
        success = registry.register_source(f"source_{i}", b"pubkey" + bytes([i]), diversity_tags={"academic"})
        assert success, f"Should allow source {i}"

    # 4th source in same window should be rejected
    success = registry.register_source("source_flood", b"pubkey_flood", diversity_tags={"academic"})
    assert not success, "Should reject flood registration"

    # After window expires, should allow again
    time.sleep(1.1)
    success = registry.register_source("source_after_window", b"pubkey_after", diversity_tags={"academic"})
